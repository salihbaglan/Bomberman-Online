...
public class MovementController : Singleton<MovementController>
{
    ...
    public GameObject EBoxWar;
    public GameObject EBox4;
    ...
    private void Update()
    {
        ...
        if (view.IsMine && !playerController.isDead) SetDirection(InputManager.Instance.Move); // Hareket yönünü ayarla       
    ...
    public void EmojiBox1()
    {
        if (view.IsMine && !playerController.isDead && !isCoroutineRunning)
        {
            StartCoroutine(DisableEBoxAfterDelay(5f));
        }
    }

    private IEnumerator DisableEBoxAfterDelay(float delay)
    {
    ...
    public void EmojiBox2()
    {
        if (view.IsMine && !playerController.isDead && !isCoroutineRunning)
        {
            StartCoroutine(DisableEBoxAfterDelay2(5f));
        }
    }

    private IEnumerator DisableEBoxAfterDelay2(float delay)
    {
        if (isCoroutineRunning)
        {
            yield break;
        ...
        isCoroutineRunning = true;
        EBox2.SetActive(true);
        yield return new WaitForSeconds(delay);
        EBox2.SetActive(false);
        isCoroutineRunning = false;
    }

    public void EmojiBoxWar()
    {
        if (view.IsMine && !playerController.isDead && !isCoroutineRunning)
        {
            StartCoroutine(DisableEBoxAfterDelayWar(5f));
        }
    }

    private IEnumerator DisableEBoxAfterDelayWar(float delay)
    {
        if (isCoroutineRunning)
        {
            yield break;
        ...
        EBoxWar.SetActive(true);
        yield return new WaitForSeconds(delay);
        EBoxWar.SetActive(false);
    }

    public void EmojiBox4()
    {
        if (view.IsMine && !playerController.isDead )
        {
            StartCoroutine(DisableEBoxAfterDelay4(5f));
        }
    }

    private IEnumerator DisableEBoxAfterDelay4(float delay)
    {
        if (isCoroutineRunning)
        {
            yield break;
        ...
        EBox4.SetActive(true);
        yield return new WaitForSeconds(delay);
        EBox4.SetActive(false);
    }
