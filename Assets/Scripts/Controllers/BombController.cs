...
public class BombController : MonoBehaviourPunCallbacks
{
        ...
        if (collision.gameObject.CompareTag("Bomb") && canIPush)
        {
            ...
            if (TryGetComponent<MovementController>(out var movementController))
            {
                ...
                if (hit.collider != null)
                {
                    ...
                    var targetPos = hit.point - direction * 0.6f;

                    // Hedef pozisyona ulaþýlabilir mi diye kontrol et
                    bool canPush = CanPushToPosition(collision.transform.position, targetPos, radius);

                    // Eðer hedef pozisyona ulaþýlabilirse bombanýn "Push" metodunu çaðýrarak itme iþlemini gerçekleþtir
                    if (canPush)
                    {
    ...
    //Bomba Ýtme Kodlarý 
    private bool CanPushToPosition(Vector2 currentPosition, Vector2 targetPosition, float radius)
    {
        // Hedef pozisyona doðru bir raycast oluþtur
        Vector2 direction = (targetPosition - currentPosition).normalized;
        float distance = Vector2.Distance(currentPosition, targetPosition);
        RaycastHit2D[] hits = Physics2D.RaycastAll(currentPosition, direction, distance);

        // Raycast ile çarpýþan nesneleri kontrol et
        foreach (var hit in hits)
        {
            // Duvar veya baþka bir engel varsa itme iþlemi yapýlamaz
            if (hit.collider.CompareTag("Wall") || hit.collider.CompareTag("DontDestroy"))
            {
                return false;
            }
        }

        // Ýtme iþlemi yapýlabilir
        return true;
    }
